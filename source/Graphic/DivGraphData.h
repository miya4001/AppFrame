...
namespace AppFrame {
  ...
  namespace Graphic {
    ...
    public:
       ...
       * @param  ySize 分割縦サイズ
       */
      DivGraphData(const std::string_view path, const int xNumber, const int yNumber, const int allNumber, const int xSize, const int ySize);
      /**
       * @brief  ファイルパスの取得
       * @return ファイルパス
       */
      ...
      /**
       * @brief  分割数の取得
       * @return 分割数(横分割数, 縦分割数, 総分割数)
       */
      inline std::tuple<int, int, int> GetNumber() const {
        return { _xNumber, _yNumber, _allNumber };
      }
      /**
       * @brief  分割サイズの取得
